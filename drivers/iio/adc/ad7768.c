...
static void ad7768_set_power_modes_info(struct ad7768_state *st)
{
	...
	for (i = 0; i < AD7768_NUM_CONFIGS; i++) {
		...
		mode_info->min_freq = mclk / (mode_info->mclk_div * DEC_1024);
		mode_info->max_freq = mclk / (mode_info->mclk_div * DEC_32);
	}
}
