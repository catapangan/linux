...
int iio_dma_resv_lock(struct dma_buf *dmabuf,
		      bool nonblock, struct ww_acquire_ctx *ctx)
{
	int ret;
	...
	if (ret) {
		...
		ret = dma_resv_lock_slow_interruptible(dmabuf->resv, ctx);
	}
...
iio_buffer_find_attachment(struct iio_dev *indio_dev, struct dma_buf *dmabuf)
{
	...
	ret = iio_dma_resv_lock(dmabuf, false, &ctx);
	if (ret)
		return ERR_PTR(ret);
				     ...
				     bool nonblock)
{
	...
	if (iio_dmabuf.bytes_used != dmabuf->size) {
		/* TODO: support buffers smaller than the DMABUF */
		ret = -EINVAL;
		goto err_dmabuf_put;
	}
	...
	ret = iio_dma_resv_lock(dmabuf, nonblock, &ctx);
	if (ret)
		goto err_fence_put;
